...
namespace Microsoft.MixedReality.Toolkit.Experimental.UI
{
    ...
    public class NonNativeKeyboard : MonoBehaviour
    {
        ...
        /// </summary>
        // --- MODIFIED ---
        // We set the default value to true to force caps lock on start.
        private bool m_IsCapslocked = true;
        ...
        public void AppendValue(KeyboardValueKey valueKey)
        {
            ...
            {
                // --- MODIFIED ---
                // We now force it to use the ShiftValue if available,
                // which is the uppercase letter.
                value = !string.IsNullOrEmpty(valueKey.ShiftValue) ? valueKey.ShiftValue : valueKey.Value;
            }

            // --- MODIFIED ---
            // We ensure shift is always false after typing a character
            // to prevent it from reverting to lowercase on the next press.
            // But we keep CapsLock true.
            if (!m_IsCapslocked)
            {
                Shift(false);
            }
        ...
        public void FunctionKey(KeyboardKeyFunc functionKey)
        {
            ...
            switch (functionKey.ButtonFunction)
            {
                ...
                case KeyboardKeyFunc.Function.Shift:
                    {
                        // --- MODIFIED ---
                        // The shift and caps lock buttons are effectively disabled
                        // by forcing the state to always be caps locked.
                        CapsLock(true);
                        break;
                    }
        ...
        public void Shift(bool newShiftState)
        {
            ...
            // --- MODIFIED ---
            // We prevent caps lock from ever being turned off.
            if (m_IsCapslocked && !newShiftState)
            {
                // m_IsCapslocked = false; // This line is commented out.
            }
        }
        ...
        {
            // --- MODIFIED ---
            // This now forces caps lock to be ON every time the state is reset.
            CapsLock(true);
        }
